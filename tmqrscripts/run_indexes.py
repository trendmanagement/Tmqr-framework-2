...
class IndexGenerationScript:
    ...
    def run_main_index_alpha_script(self):
        ...
        #             t = threading.Thread(target=self.run_through_scripts_create_index_threads, args=(instrument['instrument'], exo))
        #             t.start()

        for exo in INDEX_LIST:
            self.run_through_scripts_create_index_threads('US.ES', exo)
