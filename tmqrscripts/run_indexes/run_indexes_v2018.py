...
class IndexGenerationScript:
    ...
    def checking_alpha_then_run(self, index, current_time, last_bar_time, last_bar_time_utc, index_hedge_name, mongo_db_v1):
        ...
        try:
            ...
            if (self.reset_exo_from_beginning or self.override_time_check_run_exo or last_bar_time > alpha_sess_decision):
                ...
                for alpha in alphas_list:
                    ...
                    if self.try_run_all_exos_live_and_test \
                            or (not self.run_only_test_exos and (alpha['name'] in self.campaign_alpha_list)) \
                            or (self.run_only_test_exos and (alpha['name'] not in self.campaign_alpha_list)):
