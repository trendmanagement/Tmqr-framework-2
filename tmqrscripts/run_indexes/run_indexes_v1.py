...
class IndexGenerationScript:
    def __init__(self, override_time_check_run_exo=False, reset_exo_from_beginning=False, date_end=None,
                 override_run_alpha=False, try_run_all_exos_live_and_test=False,
                 instrument=None):
        ...
        mongo_db_v2['alpha_data'].create_index(
        ...
        self.campaign_alpha_list = self.get_campaign_alpha_list(mongo_db_v1)

        self.campaign_exo_list = self.get_campaign_exo_list(self.campaign_alpha_list, mongo_db_v2)
    ...
    def run_through_each_index_threads(self, instrument, exo_index, instrument_specific=False):
        ...
        mongo_client_v1 = MongoClient(MONGO_CONNSTR_V1, connect=False)
        mongo_db_v1 = mongo_client_v1[MONGO_EXO_DB_V1]

        ExoClass = INDEX_LIST[exo_index]['class']
        ...
        if self.try_run_all_exos_live_and_test or index_hedge_name in self.campaign_exo_list:
            ...
            try:
                ...
                if self.reset_exo_from_beginning:
                    ...
                    self.run_index(index, ct['current_time_utc'], index_hedge_name, mongo_db_v2, creating_index=True)

                    self.checking_alpha_then_run(index, ct['current_time'], ct['current_time_utc'], index_hedge_name,
                                                 mongo_db_v1, mongo_db_v2)
                else:
                    ...
                    index_from_db = mongo_db_v2['index_data'].find_one({'name': index_hedge_name}, {'context': 1})

                    if index_from_db == None or not 'index_update_time' in index_from_db['context']:
                        self.run_index(index, ct['current_time_utc'], index_hedge_name, mongo_db_v2)
                    else:
                        last_index_update_time = self.time_to_utc_from_none(
                            index_from_db['context']['index_update_time'])
                        ...
                        if self.override_time_check_run_exo or (ct['current_time'].weekday() < 5 and \
                                                                        ((ct[
                                                                         ...
                                                                         or (ct[
                                                                                 'current_time'] >= sess_exec and last_index_update_time < sess_exec))):
                            self.run_index(index, ct['current_time_utc'], index_hedge_name, mongo_db_v2)
    ...
    def run_index(self, index, update_time, index_hedge_name, mongo_db_v2, creating_index=False):
        '''
        ...
        try:
            mongo_db_v2['index_data'].update_one({'name': index_hedge_name},
        ...
        #         print("ran index")
        index.save()
        print("saved index {0}".format(index_hedge_name))
    ...
    def checking_alpha_then_run(self, index, current_time, current_time_utc, index_hedge_name, mongo_db_v1,
        ...
        #         print('alpha_sess_decision {0} {1} {2}'.format(alpha_sess_start, alpha_sess_decision, alpha_sess_exec))

        if self.reset_exo_from_beginning or self.override_time_check_run_exo or current_time >= alpha_sess_decision:
            alphas_list = list(mongo_db_v2['alpha_data'].find({'context.index_hedge_name': index_hedge_name},
                                                              {'name': 1, 'context': 1}))
                    ...
                    if 'v1_alphas' in alpha['context']:
                        ...
                        if swarm_list:
                            ...
                            for swarm in swarm_list:
                                ...
                                if not v1_alpha_ok:
                                    current_time_utc = self.time_to_utc_from_local_tz(
                                        datetime.combine(earliest_date, time(0, 0, 0)), index.session.tz.zone)

                    if not 'alpha_update_time' in alpha['context']:
                        # t = threading.Thread(target=self.run_alpha, args=(alpha['name'], current_time_utc))
                        # t.start()
                        self.run_alpha(alpha['name'], current_time_utc, mongo_db_v2)
    ...
    def run_alpha(self, alpha_name, update_time, mongo_db_v2):
        '''
        alpha run and save
        :param alpha_name: 
        :param update_time: 
        ...
        mongo_db_v2['alpha_data'].update_one({'name': alpha_name},
        ...
        try:
            ...
            #             print('ran alpha')
            saved_alpha.save()
            print("saved alpha {0}".format(alpha_name))
        except:
            pass
    ...
    def get_campaign_alpha_list(self, mongo_db_v1):
        '''
    ...
    def get_campaign_exo_list(self, final_alpha_list, mongo_db_v2):
        '''
        this gets the full list of exos that the current active campaigns use
        :return: the list of exos that the campaigns use
        '''

        alpha_list_mongo = mongo_db_v2['alpha_data'].find({'name': {'$in': final_alpha_list}}, {'context': 1})
