            ...
            while not os.path.isfile(ctrl_file):
                if last_refresh_date is None or int((datetime.datetime.now() - last_refresh_date).total_seconds()/60) >= historical_refresh_interval:
                    ...
                    for iq_ticker_b, watch_rec in iq_watchlist.items():
                        ...
                        data_start = watch_rec['last_date_utc'].astimezone(timezone_est) - datetime.timedelta(minutes=1)
                        is_delayed = watch_rec['iqfeed_is_delayed']
                        ...
                        if is_delayed:
                            try:
                                ...
                                if len(bars_data) == 0:
                                    # Unexpected behavior which means that IQFeed freezes, usualy it throws NO DATA exception, when there is no data
                                    log.error("Suspected IQConnect.exe crash, it always must return bars or raise NoData exception")
                                    sys.exit(-3)
