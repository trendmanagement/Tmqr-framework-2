...
class ES_EXOSemiFuture_DynKel_20_80_Longs(IndexEXOBase):
    _description_short = "EXO Vanilla DeltaTargeting DynKel Delta Risk Reversal"
    _description_long = ""

    _index_name = "EXOSemiFuture_ES_DynKel_20_80_Longs"
    ...
    def calc_exo_logic(self):
        ...
        typical_avg = typical_px.rolling(50).mean()

        keltner_direction = typical_avg > typical_avg.shift()
        #         keltner_direction = typical_px > typical_avg.shift() #and typical_avg > typical_avg.shift()
    ...
    def construct_position(self, dt, pos, logic_df):
        ...
        if logic_df['keltner_direction_current'] == True:
            ...
            fut, opt_chain = self.dm.chains_options_get(self.instrument, dt, opt_codes=['EW4'])
            pos.add_transaction(dt, opt_chain.find(dt, 0.1, 'C', how='delta'), 1.0)
            ...
            fut_next, opt_chain_next = self.dm.chains_options_get(self.instrument, dt, opt_codes=['EW4'])
            pos.add_transaction(dt, opt_chain_next.find(dt, 0.3, 'P', how='delta'), 2.0)
            # pass